...
def main():
    ...
    if config.MODEL.FREEZE_SOME:
        for name, param in model.named_parameters():
            to_train = re.search(config.MODEL.TRAINABLES, name)
            if not to_train:
    ...
    criterion = build_criterion(config, True)
    criterion.cuda()
    criterion_eval = build_criterion(config, False)
    criterion_eval.cuda()
...
#        if (epoch + 1) % 20 == 0:
#            save_model_on_master(
#                model, args.distributed, final_output_dir, 'model_'+str(epoch)+'.pth'
#            )
