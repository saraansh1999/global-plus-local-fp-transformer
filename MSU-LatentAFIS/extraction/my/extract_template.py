...
from inceptionv3Version import Inferencer
...
class FeatureExtraction_Rolled:
	def __init__(self, patch_types=None, des_model_dirs=None, minu_model_dir=None):
		...
		if args.mode == "gt" and self.des_model_dirs is not None:
			self.des_models = []
		...
		if args.mode == "infer":
			self.patch_size = 96
			self.des_models = Inferencer(args.gpu)
	...
	def feature_extraction_single_rolled(self, img_file, output_dir=None, ppi=500):
		...
		patches, des = descriptor.minutiae_descriptor_extraction(img, mnt, self.patch_types, self.des_models, self.patchIndexV,
														batch_size=256, patch_size=self.patch_size, mode=args.mode)
		stop = timeit.default_timer()
	...
	def feature_extraction(self, image_dir=None, img_type='bmp', template_dir=None, enhancement=False):
		...
		for i, img_file in enumerate(img_files):
			...
			if template_dir is not None:
				# fname = template_dir + img_name + '.dat'
				fname = os.path.join(template_dir, img_file[len(image_dir):])
...
def parse_arguments(argv):
	...
	parser.add_argument('--mode', type=str, help='gt / infer')
