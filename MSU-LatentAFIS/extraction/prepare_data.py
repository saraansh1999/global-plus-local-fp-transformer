...
def extract_minutiae_cylinder(img_input, minutiae_input, ROI=None, num_ori=6, angle=None, processing=None):
    # for the latent or the low quality rolled print
...
if __name__ == '__main__':
    ...
    input_dir = '/scratch/additya/fps/segmented/images/NIST302/'
    minu_dir = '/scratch/additya/fps/segmented/minutiae/NIST302'
    output_dir = '/scratch/additya/fps/segmented/minutiae_maps/NIST302'
    ...
    for root, dirs, files in os.walk(input_dir):
        for file in files:
            ...
            mpath = os.path.join(minu_dir, fpath[len(input_dir):].lstrip('/'))
            mpath = '.'.join(mpath.split('.')[:-1] + ['npy'])
            opath = os.path.join(output_dir, mpath[len(minu_dir):].lstrip('/'))
            fs.append([fpath, mpath, opath])
            # print(fpath, mpath, opath)

    print('total:', len(fs))
    # exit(0)

    for idx, f in enumerate(fs):
        fpath, mpath, opath = f[0], f[1], f[2]
        odir = '/'.join(opath.split('/')[:-1])
        ...
        # print(minu.shape)
        if len(minu.shape) == 1:
            minu = np.reshape(minu, (1, 4))
        minu[:, 0] = (minu[:, 0] / im.shape[1]) * 128
        minu[:, 1] = (minu[:, 1] / im.shape[0]) * 128
        im = cv2.resize(im, (128, 128))
        _, _, cylinder = extract_minutiae_cylinder(im, minu)
        np.save(opath, cylinder)
        if idx % 500 == 0:
            print('done:', idx)
