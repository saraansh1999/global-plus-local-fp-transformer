...
def pad_and_resize(img, s1, s2):
        # pad and resize
        sz = img.shape
        inter_sz = (max(int(1.15 * s1), img.shape[0]), max(int(1.15 * s2), img.shape[1]))
        inter_img = np.ones((inter_sz[0], inter_sz[1]), dtype=np.int32) * 255
        margin_h = (inter_sz[0] - sz[0]) // 2
        margin_w = (inter_sz[1] - sz[1]) // 2
        inter_img[margin_h:margin_h+sz[0], margin_w:margin_w+sz[1]] = img

        inter_img = np.stack([inter_img], axis=0)
        img = center_crop(inter_img, (int(1.15 * s1), int(1.15 * s2)))

        img = resize(img, (int(1.07 * s1), int(1.07 * s2)))
        img = center_crop(img, (s1, s2))
