...
def get_patch_location(patch_type=1):
	if patch_type == 1: 
		...
		# print(">>>>>>>>>>>>>>> CHANGE BACK TO 32:128")
		x = np.array(xrange(32, 128))		#40:232 for 192		#32:128 for 96
...
def extract_patches(minutiae, img, patchIndexV, patch_type=1, patch_size=96):
	...
	for i in xrange(num_minu):
		...
		'''
		minx, maxx, miny, maxy = np.min(xv), np.max(xv), np.min(yv), np.max(yv)
...
def cls_descriptor_extraction(patches, models, batch_size=128):
                k = 0	# only one patch type
                embedding_size = models[k].embedding_size
                nrof_patches = patches.shape[0]
...
def minutiae_descriptor_extraction(img, minutiae, patch_types, models, patchIndexV, batch_size=128, patch_size=96, mode="infer"):
	des = []
	times_des = []
	times_patches = []
	# print("hello!", mode)
	...
	if 'gt' in mode:
		...
		for k, patch_type in enumerate(patch_types):
			...
			times_patches.append(timeit.default_timer() - start_time)
			embedding_size = models[k].embedding_size
			...
			for i in range(nrof_batches):
				...
				emb_array[start_index:end_index, :] = models[k].run(patches_batch)
				times_des.append(timeit.default_timer() - start)
			for i in range(nrof_patches):
				norm = np.linalg.norm(emb_array[i, :]) + 0.0000001
				emb_array[i, :] = emb_array[i, :] / norm
		...
		#return np.array(patches_all), np.array(des)
		return np.array(patches_all), np.array(des), np.array(times_patches), np.array(times_des)
	elif mode == "infer" or mode == "patches":
		patches = []
		for k, patch_type in enumerate(patch_types):
				patches.append(extract_patches(minutiae, img, patchIndexV, patch_type=patch_type, patch_size=patch_size))
		patches = np.array(patches)
		if mode == "patches":
			return patches
		des = models.test(patches)

	patches, des = np.array(patches), np.array(des)
	# print('descriptor', patches.shape, des.shape)
	return patches, des
