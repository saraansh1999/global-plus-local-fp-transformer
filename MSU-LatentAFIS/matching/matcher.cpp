...
float Matcher::One2One_minutiae_matching(MinutiaeTemplate &latent_minu_template, MinutiaeTemplate &rolled_minu_template, bool save_corr, string corr_file)
{
	...
	cout << "topN " << simi << " " << latent_minu_template.m_minutiae[ind_1].x << ' ' << latent_minu_template.m_minutiae[ind_1].y << " " << latent_minu_template.m_minutiae[ind_1].ori << " " << rolled_minu_template.m_minutiae[ind_2].x << ' ' << rolled_minu_template.m_minutiae[ind_2].y << " " << rolled_minu_template.m_minutiae[ind_2].ori << " " << endl;
        score += simi;
    } // cout << corr.size() << endl << endl;
    // cout << "initial score: " << score << endl;
	//cout << endl;
     // step 4: remove false correspondences using two graph matching
    int d_thr = 20; // original is 30
    vector<tuple<float, int, int>> corr2 = LSS_R_Fast2_Dist_eigen(corr, latent_minu_template, rolled_minu_template, d_thr);      //this is original
    float score2 = 0.0;
    for(i=0; i<corr2.size(); ++i) {
	score2 += get<0>(corr2[i]);
	cout << "corr2 " << get<0>(corr2[i]) << ' ' << latent_minu_template.m_minutiae[get<1>(corr2[i])].x << ' ' << latent_minu_template.m_minutiae[get<1>(corr2[i])].y << " " << latent_minu_template.m_minutiae[get<1>(corr2[i])].ori << " " << rolled_minu_template.m_minutiae[get<2>(corr2[i])].x << ' ' << rolled_minu_template.m_minutiae[get<2>(corr2[i])].y << " " << rolled_minu_template.m_minutiae[get<2>(corr2[i])].ori << endl;
    }
    // cout << "Corr2 score:" << ' ' << corr2.size() <<endl << endl;
	// vector<tuple<float, int, int>> corr2 = Des_Based_Second_Order_Filtering(corr, latent_minu_template, rolled_minu_template, d_thr, aa, bb);      
...
*/ 	// uncomment for original
    ...
    {
	cout << "corr3 " << get<0>(corr3[i]) << ' ' << latent_minu_template.m_minutiae[get<1>(corr3[i])].x << ' ' << latent_minu_template.m_minutiae[get<1>(corr3[i])].y << " 0 " << rolled_minu_template.m_minutiae[get<2>(corr3[i])].x << ' ' << rolled_minu_template.m_minutiae[get<2>(corr3[i])].y << " 0" << endl;
        score3 += get<0>(corr3[i]);
    }
    // cout << score3 << endl;
    return score3;
...
float Matcher::Our_Matching(string file1, string file2)
{
    // cout << "Loading..." << endl;
    MinutiaeTemplate template1, template2;
    Our_Loader(file1, template1);
    // cout << "Loaded 1" << endl;
    Our_Loader(file2, template2);
    // cout << "Loaded 2" << endl;

    // vector<float> score;
    float ret = One2One_minutiae_matching(template1, template2, false, "notneeded");
    // cout << ret << endl;
    return ret;
}

int Matcher::Our_Matching_Time(string file1, string file2)
{
    // cout << "Loading..." << endl;
    MinutiaeTemplate template1, template2;
    Our_Loader(file1, template1);
    ...
    // duration<double, std::microseconds> time_span;

    int warmup_iters = 50;
    for(int i=0; i<warmup_iters; ++i) ret = One2One_minutiae_matching(template1, template2, false, "notneeded"); 

    int repetitions = 300;
    for(int i=0; i<repetitions; ++i) {
        t_start = high_resolution_clock::now();
        ret = One2One_minutiae_matching(template1, template2, false, "notneeded");
        t_end = high_resolution_clock::now();
        // time_span = t_end - t_start;
        double c_time = std::chrono::duration_cast<std::chrono::microseconds>(t_end - t_start).count();
        times.push_back(c_time);
	// std::cout << c_time << std::endl;
    }
    // std::cout << "times size:" << times.size() << std::endl; // << ' ' << times[0] << std::endl;
    // std::cout << times[0].count() << std::endl;
    double mean_time = 0;
    for(int i=0; i<times.size(); ++i) mean_time += times[i];
// micro seconds me avg time
    std::cout << mean_time / times.size() << std::endl;
...
int Matcher::Our_Loader(string tname, MinutiaeTemplate & fp_template)
{
    fp_template.release();
    const short Max_Nrof_Minutiae = 2*1000; // including virtual minutiae. We only consider top 1000 minutiae including both real and virtual minutiae for each template.
    const short Max_Des_Length = 192;
    ...
    is.read(reinterpret_cast<char*>(&blkH),sizeof(short));
    is.read(reinterpret_cast<char*>(&blkW),sizeof(short));
    is.read(reinterpret_cast<char*>(&nrof_minu_template),sizeof(short));
    // cout << h << " " << w << " " << blkH << " " << (nrof_minu_template) << " " << blkW << endl;
    if(blkH>50)
        blkH = 50;
    if(blkW>50)
        blkW = 50;
    for(i=0;i<int(nrof_minu_template); ++i)
    {
        is.read(reinterpret_cast<char*>(&nrof_minutiae),sizeof(short));
        // cout << nrof_minutiae << endl;
        if(nrof_minutiae<=0)
            continue;
        if(nrof_minutiae>Max_Nrof_Minutiae)
        {
...
//        for(int j = 0; j < nrof_minutiae; j++) cout << j << " " << x[j] << " " << y[j] << " " << ori[j] << endl;
        // cout << x[0] << " " << y[0] << " " << ori[0] <<  " " << des[0] << endl;

        fp_template = MinutiaeTemplate(nrof_minutiae,x,y,ori,des_len,des,blkH, blkW, oimg);
    }
	// cout << endl << endl;
    return 0;
};
...
vector<tuple<float, int, int>>  Matcher::LSS_R_Fast2_Dist_eigen(vector<tuple<float, int, int>> &corr, SingleTemplate & latent_template, SingleTemplate & rolled_template, float d_thr)
{
	...
	if((i == 5 && j == 9) or (i == 9 && j == 5)) {
		// cout << "5-9-H dist " << dist_1 << ' ' << dist_2 << ' ' << dist << endl;
	}
...
//		if(get<1>(corr[i]) == 12 && get<2>(corr[i]) == 2) {
//			cout << i << " dist = " << dist << " " << "12 2 " << get<1>(corr[j]) << ' ' << get<2>(corr[j]) << endl;
//		}


            H[i*num+j] = (d_thr-dist)/(25.0);
            if(H[i*num+j]>1)
                H[i*num+j] = 1.0;
            else if(H[i*num+j]<0)
                H[i*num+j] = 0.0;
...
//	if(ind == 9) cout << ind << ' ' << S[ind] << ' ' << i << endl ;
        ...
        else
        {
            ...
            for(j=0;j<selected_ind.size(); ++j)
            {
                if(H[ind*num+selected_ind[j]]<0.00001)
                {
		//	if(ind == 9) cout << "compatible with selected " << selected_ind[j] << " " << H[ind*num+selected_ind[j]] << " " << get<1>(corr[selected_ind[j]]) << " " << get<2>(corr[selected_ind[j]]) << endl;
                    found = 1;
                    break;
                }
            }
		// if(ind == 9) cout << "found: " << found << " " << get<1>(corr[ind]) << " " << get<2>(corr[ind]) << endl;
            if(found==0)
            {
                selected_ind.push_back(ind);
                new_corr.push_back(make_tuple(get<0>(corr[ind]),get<1>(corr[ind]),get<2>(corr[ind])));
...
//	cout << "new corr size: " << i << ' ' << new_corr.size() << endl;
    }
...
double get_cosine(Matrix<float, Eigen::Dynamic, Eigen::Dynamic> a, Matrix<float, Eigen::Dynamic, Eigen::Dynamic> b){
    MatrixXf aa = a*a.transpose();
    MatrixXf bb = b*b.transpose();
    MatrixXf ab = a*b.transpose();
    return ab(0, 0) / (sqrt(aa(0, 0))*sqrt(bb(0, 0)));
}

vector<tuple<float, int, int>>  Matcher::Des_Based_Second_Order_Filtering(vector<tuple<float, int, int>> &corr, SingleTemplate & latent_template, SingleTemplate & rolled_template, float d_thr, Matrix<float, Eigen::Dynamic, Eigen::Dynamic> AA, Matrix<float, Eigen::Dynamic, Eigen::Dynamic> BB)
{
    int num = corr.size();
    float *H = new float [num*num]();
    ...
    for(i=0; i<num-1; ++i)
    {
        ...
        for(j=i+1; j<num;++j)
        {
           ...
           if(dist>d_thr)
            ...
            // HERE WE ADD des BASED FILTERING
            des_1 = get_cosine(AA.row(get<1>(corr[i])), AA.row(get<1>(corr[j])));
            des_2 = get_cosine(BB.row(get<2>(corr[i])), BB.row(get<2>(corr[j])));
            des_dist = fabs(des_1 - des_2);
            // cout << des_1 << " " << des_2 << " " << des_dist << endl;
            double des_thr = 0.1;
            if(des_dist>des_thr)
                continue;
            ////////////////////////////////////////////////
