...
public:
    ...
    float Our_Matching(string file1, string file2);
    int Our_Matching_Time(string file1, string file2);
    int Our_Loader(string tname, MinutiaeTemplate & fp_template);
...
private:
    ...
    vector<tuple<float, int, int>>  Des_Based_Second_Order_Filtering(vector<tuple<float, int, int>> &corr, SingleTemplate & latent_template, SingleTemplate & rolled_template, float d_thr, Matrix<float, Eigen::Dynamic, Eigen::Dynamic> aa, Matrix<float, Eigen::Dynamic, Eigen::Dynamic> bb);
